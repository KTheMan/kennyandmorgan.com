...
3. Configure your registry IDs in `.env`:
```
...
Fetch items from all registries.

**Query Parameters:**
- `store` (optional): Filter by store name (`amazon`, `target`, `crateandbarrel`, or `all`)
...
Each scraper module:
1. Attempts to fetch real registry data when configured with a valid registry ID
2. Falls back to mock data if:
   - No registry ID is configured
   - The registry ID is the default placeholder value
