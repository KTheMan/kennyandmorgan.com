...
create or replace function public.list_registry_items(store_filter text default null, include_unavailable boolean default false)
returns jsonb
language sql
security definer
set search_path = public
as $$
    select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'image', item.image,
        'price', item.price,
        'wantedQuantity', item.wanted_quantity,
        'purchasedQuantity', item.purchased_quantity,
        'fastPollActive', false,
        'available', item.available
    ) order by item.sort_order asc, item.name asc), '[]'::jsonb)
    from public.registry_items item
    where (store_filter is null or lower(item.store) = lower(store_filter))
      and (include_unavailable or item.available = true);
$$;
...
revoke all on public.registry_items from anon, authenticated;
...
grant execute on function public.list_registry_items(text, boolean) to anon, authenticated;
