...
-- Multiple venues, each with a shareable slug and its own PIN. All access
-- goes through that edge function (service role) — never directly from
-- anon/authenticated clients, so PIN checks stay rate-limited.
...
-- 4-digit PIN (10,000 possibilities) can't be brute-forced.
