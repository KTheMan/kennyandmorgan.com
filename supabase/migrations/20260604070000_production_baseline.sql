do $$
begin
    if to_regclass('public.access_passwords') is not null
        or to_regclass('public.guests') is not null
        or to_regclass('public.rsvp_submissions') is not null
        or to_regclass('public.registry_items') is not null
        or to_regclass('public.rehearsal_lunch_rsvps') is not null then
        raise exception 'Production baseline migration must be marked applied, not executed against an existing database.';
    end if;
end;
$$;
